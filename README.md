...
**kitty** needs remote control for the default side-panel mode — two lines in `~/.config/kitty/kitty.conf`, then restart kitty:
...
Without it, rotpilot falls back to a separate kitty window. Switch explicitly with `rotpilot window panel|window`. In kitty the panel width is `panelBias` in the config (default 33%) and the tab temporarily switches to the `splits` layout; in Ghostty splits are 50/50 (no resize API yet) and the big headline text renders normal-sized (Ghostty hasn't implemented the text-sizing protocol yet).
...
- **macOS**
- **[kitty](https://sw.kovidgoyal.net/kitty/)** or **[Ghostty](https://ghostty.org) ≥1.3** — the two terminals with real APIs for all three things rotpilot needs: graphics, focus control, and pane spawning. kitty does it over its remote-control socket; Ghostty over its AppleScript dictionary (verified: 480/480 frames accepted at 24fps).
...
- **`instagram`** — your actual Reels. **Opt-in only, at your own risk**: automating Instagram violates Meta's ToS and accounts can get flagged. rotpilot only ever scrolls (never likes/follows/comments) and uses real headful Chrome with a clean fingerprint, but use a burner account. You must set `"allowInstagram": true` in the config by hand; there is no flag to fat-finger.
...
```
  ...
  while you rotted: 3 edits · 2 commands · 1 question waiting
...
You watched reels; your memory watched Claude. Weeks later, in any repo, you can ask what happened while you weren't looking — and get a straight, funny answer, not a log dump:
...
**Consent is two separate switches**: the API key (`rotpilot engram key` — hidden prompt, stored `0600`; or `ENGRAM_API_KEY`) enables recap reads. Transcript ingestion — your prompts and Claude's work, which can include code, leaving the machine for your project — only happens after `rotpilot engram transcripts on`, and stops with `off`.

Setup is two topics, one key, one opt-in: `rotpilot engram` prints the exact design (both topics unbounded, user + property `project` — the names and descriptions are the extraction prompts); `rotpilot engram check` verifies the pipe live — auth → conversation ingestion → search.
...
MIT. Not affiliated with Anthropic, Google, Meta, or your dopamine receptors.
